# Start page: http://www.pythonchallenge.com/pc/def/equality.html
...
def is_protected_letter(slice: str) -> bool:
    return re.search(pattern, slice)


res = ''
for i in range(BODYGUARDS_COUNT + 1, len(text) + 1 - (BODYGUARDS_COUNT + 1)):
    slice = text[i - (BODYGUARDS_COUNT + 1): i + (BODYGUARDS_COUNT + 1) + 1]
    if is_protected_letter(slice):
        res += text[i]
